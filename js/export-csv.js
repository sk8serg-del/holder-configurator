/*
 * export-csv.js — генерация CSV координат для Inventor (правило HolderFromCSV, v3).
 *
 ...
 *   mark;{Ø зенковки-метки};{угол зенковки}
 ...
 *           markX, markY — метка-ориентир (маленькая зенковка, см. HC.MARK_D/MARK_ANGLE):
 *               ставится только у ДЕТАЛЕЙ (не у контрольных отверстий) с пазом,
 *               всегда с одной и той же стороны паза, в 2 мм от посадки и в 2 мм
 *               от паза — нужна, чтобы отличать похожие по размеру детали и
 *               (для форм, где это важно) метить ориентацию при сборке.
 *               Пусто, если паза нет или метка не применима.
 */
(function (g) {
  "use strict";
  var HC = (g.HC = g.HC || {});
  ...
  // Геометрия паза (абс. угол, длина, ширина, точка метки) — та же логика,
  // что в render.js.
  function slotGeom(f) {
  ...
  HC.buildCSV = function (order) {
    ...
    L.push(["mark", num(HC.MARK_D), num(HC.MARK_ANGLE)].join(";"));
    L.push("columns;kind;type;cx;cy;d;w;h;chamfer;rot;seatD;caDia;seatGap;caInset;slot;slotAngle;slotL;slotW;depth;markX;markY");
